...
param(
    ...
    [string]$OutputRelativePath = 'artifacts/atlas-chat-refonte/server-candidate-singlefile'
)
...
$candidateArchive = Join-Path $candidateRoot 'atlas-chat-workspace-api-linux-x64-schema7.tar.gz'
...
$candidateTests = @(
    @{ Name = 'chat-v2-mysql'; Count = 86; File = 'chat-v2-mysql.log' },
    @{ Name = 'chat-rich-media'; Count = 105; File = 'chat-rich-media.log' },
    @{ Name = 'chat-v1-bridge-mysql'; Count = 238; File = 'chat-v1-bridge-mysql.log' }
)
$candidateEvidence = @()
foreach ($candidateTest in $candidateTests) {
    $candidateLog = Join-Path $candidateRepo ('artifacts/chat-v2-validation/' + $candidateTest.File)
    if (-not (Test-Path -LiteralPath $candidateLog -PathType Leaf) -or
        -not (Get-Content -LiteralPath $candidateLog -Raw).Contains(('PASS: ' + $candidateTest.Count + ' assertions'))) {
        throw "Expected validated test evidence is missing: $($candidateTest.Name)"
    }
    $candidateEvidence += [ordered]@{ name=$candidateTest.Name; assertions=$candidateTest.Count;
...
if ($candidateMigrationRows.Count -ne 7 -or $candidateMigrationRows[-1].name -ne '0007_chat_workspace.sql') {
    throw 'This candidate must contain the seven verified migrations, including the additive chat workspace.'
}
...
$candidateManifest = [ordered]@{
    formatVersion=1; purpose='Atlas chat workspace API candidate'; builtAtUtc=[DateTimeOffset]::UtcNow.ToString('O');
    runtime='linux-x64'; selfContained=$true; singleFile=$true; publishTrimmed=$false; nativeLibrariesForSelfExtract=$false;
    schemaVersion=7; apiVersions=@(1,2); deployment=$false;
    productionAccess=$false; productionServicesRestarted=$false; nativeExecutionOnLinuxVerified=$false;
    requiredProductionConfiguration=@('WOTLK_LAUNCHER_MAX_SCHEMA_VERSION=7','WOTLK_CHAT_MEDIA_ROOT=<private writable directory>');
    ...
    boundaries=@('This archive has not been deployed.',
        'Migration0006 is unchanged; migration0007 adds workspace tables and backfills direct history.',
        'An old server binary rejects a schema history containing an unknown migration; binary-only rollback after migration7 is not supported.',
