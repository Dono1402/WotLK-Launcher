#!/usr/bin/env python3
"""Stage or publish only the 1.7.2 client; never restart a service."""
import fcntl
import hashlib
import json
import os
...
VERSION = '1.7.2'
ROOT = Path('/opt/atlas-shop-releases/gold-1.7.2-20260912/launcher')
UPLOAD = Path('/tmp/atlas-launcher-hotfix-172-20260912')
...
def atomic_copy(source, target, mode=0o644):
    ...
    descriptor, temporary = tempfile.mkstemp(prefix='.atlas172-', dir=target.parent)
    try:
...
def prepare():
    ...
    if json.loads((PUBLIC / 'launcher-update.json').read_text())['version'] != '1.7.1':
    ...
    for suffix in ('', '.en'):
        ...
        if note['id'] != 'atlas-launcher-1-7-2': raise RuntimeError('Unexpected patch note')
        ...
        for directory in (prepared / 'store/v1.7.2', prepared / 'metadata/releases/v1.7.2'):
    ...
    shutil.copyfile(armory, prepared / 'store/v1.7.2/armory-runtime.zip')
    metadata_path = prepared / 'metadata/releases/v1.7.2/release.json'
    metadata = json.loads(metadata_path.read_text())
    metadata.update(publishedRoot=str(PUBLIC), artifactStore=str(STORE / 'v1.7.2'),
    ...
    print('PASS: signed 1.7.2 prepared privately; public baseline unchanged.', flush=True)


def immutable(source, target):
    if target.exists() or target.is_symlink(): raise RuntimeError('Immutable destination exists')
    stage = Path(tempfile.mkdtemp(prefix='.atlas172-', dir=target.parent))
...
def publish():
    from release_runtime import verify_runtime
    verify_runtime(True)
    if not json.loads((ROOT.parent / 'purchases-enabled.json').read_text())['enabled']: raise RuntimeError('Backend activation required')
    ...
    changed = []
    try:
        immutable(prepared / 'public/releases/1.7.2', PUBLIC / 'releases/1.7.2')
        immutable(prepared / 'store/v1.7.2', STORE / 'v1.7.2')
        immutable(prepared / 'metadata/releases/v1.7.2', METADATA / 'releases/v1.7.2')
        for name, input_name in [('WotLK-Launcher.exe', 'WotLK-Launcher.exe'),
                                 ('WotLK-Launcher-Installer.exe', 'AtlasLauncherSetup.exe')]:
            download('https://animeclub.fr/wotlk/launcher/releases/1.7.2/' + name, inputs[input_name])
        operations = [(manifest, METADATA / 'current/launcher-update.json'),
            (prepared / 'public/releases/1.7.2/WotLK-Launcher.exe', PUBLIC / 'WotLK-Launcher.exe'),
            (prepared / 'public/releases/1.7.2/WotLK-Launcher-Installer.exe', PUBLIC / 'WotLK-Launcher-Installer.exe'),
            (prepared / 'public/releases/1.7.2/WotLK-Launcher-Installer.exe', PUBLIC / 'AtlasLauncherSetup.exe'),
        ...
        print('PASS: 1.7.2 public, downloads verified, all service processes unchanged.', flush=True)
