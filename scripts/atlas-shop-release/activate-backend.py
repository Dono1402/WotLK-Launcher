...
def main():
    ...
    if query('SELECT version FROM arthas_auth.atlas_launcher_schema_history ORDER BY version;') != [[str(x)] for x in range(1, 9)]:
    ...
    try:
        ...
        for name in ('hermesproxy-wotlk', 'wotlk-launcher-api', WORLD_SERVICE):
            subprocess.run(['systemctl', 'stop', name], check=True, timeout=180)
            stopped.append(name)
            if service(name)['MainPID'] != '0': raise RuntimeError('Service did not stop')
        phase('fresh-backup-before-migration')
        subprocess.run(['python3', str(ROOT / 'scripts/backup.py'), '--after-stop'], check=True, timeout=180)
        ...
        next_link = WORLD / 'server/.etc-production-activate-170'
        next_link.symlink_to(ops['worldConfigLinkAfter'], target_is_directory=True)
        os.replace(next_link, link)
        subprocess.run(['systemctl', 'daemon-reload'], check=True)
        for name, binary in ((WORLD_SERVICE, WORLD / 'server/bin/worldserver'), ('hermesproxy-wotlk', HERMES / 'HermesProxy'), ('wotlk-launcher-api', API / 'WotLK.Launcher.Server')):
        ...
        if service(WORLD_SERVICE)['WorkingDirectory'] != ops['worldWorkingDirectoryPreserved']: raise RuntimeError('World working directory changed')
        phase('migrating-api-with-purchases-closed')
        api_started = True
        subprocess.run(['systemctl', 'start', 'wotlk-launcher-api'], check=True)
        wait_for('API healthy with schema 13', health, 90)
        ...
        if [int(x[0]) for x in history] != list(range(1, 14)) or history[:8] != plan['activeBefore']['database']['migrationHistory']:
        ...
        wait_for('World listening on 4000 with protocol 2 heartbeat', world_ready, 300)
        phase('starting-hermes')
        subprocess.run(['systemctl', 'start', 'hermesproxy-wotlk'], check=True)
        wait_for('Hermes process active', lambda: service('hermesproxy-wotlk')['ActiveState'] == 'active' and service('hermesproxy-wotlk')['MainPID'] != '0', 60)
        ...
        failures = []
        for name in ('hermesproxy-wotlk', WORLD_SERVICE, 'wotlk-launcher-api'):
            try: subprocess.run(['systemctl', 'stop', name], check=True, timeout=180)
        ...
        subprocess.run(['systemctl', 'daemon-reload'], check=True)
        for name in ('wotlk-launcher-api', WORLD_SERVICE, 'hermesproxy-wotlk'):
            try: subprocess.run(['systemctl', 'start', name], check=True, timeout=180)
            except BaseException as failure: failures.append(name + ':' + type(failure).__name__)
