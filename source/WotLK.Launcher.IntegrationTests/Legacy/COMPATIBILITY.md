...
| Play / Addons | Addon work uses the same global cancellation source and disables Play through `SetBusy`. | The current coordinator permits Addons alongside Play; the UI shows the running game and advises `/reload` after success. |
...
- Play uses an independent single-flight lock. It may coexist with read-only
  Verify when the local client is playable, and with addon maintenance. Mutating
  game or launcher maintenance is rejected immediately while Play is active;
  every maintenance operation remains rejected while Verify is active.
