"""Shared helpers for the explicitly authorized Atlas 1.7.2 maintenance."""
import json
import os
...
def atomic_copy(source, target, mode=None):
    ...
    fd, name = tempfile.mkstemp(prefix='.' + target.name + '.atlas172-', dir=target.parent)
    temporary = Path(name)
    try:
...
def world_ready():
    ...
    if query("SELECT COUNT(*) FROM arthas_auth.atlas_shop_conversion_health WHERE realm_id=1 AND protocol=1 AND copper_per_cent=10000 AND character_database='arthas_chars' AND last_seen_at BETWEEN UTC_TIMESTAMP(6)-INTERVAL 30 SECOND AND UTC_TIMESTAMP(6)+INTERVAL 5 SECOND;") != [['1']]: return False
    try:
...
def verify_runtime(purchases):
    ...
    expected = {WORLD_SERVICE: (WORLD / 'build/worldserver', plan['worldSha256']),
        'wotlk-launcher-api': (API / 'WotLK.Launcher.Server', plan['files']['WotLK.Launcher.Server'])}
    result = {}
    for name, (path, sha) in expected.items():
        ...
        result[name] = state
    for name in plan['preservedServices']:
        state = service(name)
        expected = plan['activeBefore']['services'][name]
        if state['MainPID'] != expected['MainPID'] or state['ActiveState'] != 'active' or digest('/proc/' + state['MainPID'] + '/exe') != expected['ExecutableSha256']:
            raise RuntimeError('Preserved process changed: ' + name)
        result[name] = state
    if service(WORLD_SERVICE)['WorkingDirectory'] != plan['activeBefore']['services'][WORLD_SERVICE]['WorkingDirectory']:
        raise RuntimeError('World working directory changed')
    for path, sha in plan['configurationFiles'].items():
        if digest(path) != sha: raise RuntimeError('Active candidate configuration changed')
    if str((WORLD / 'server/etc').resolve(strict=True)) != plan['worldConfigLinkAfter']:
        raise RuntimeError('World configuration link differs')
    ...
    flags = {'WOTLK_LAUNCHER_MAX_SCHEMA_VERSION': '14', 'AtlasShop__Purchases__AccountServicesEnabled': 'true',
        'AtlasShop__Purchases__RenameEnabled': 'true' if purchases else 'false',
        'AtlasShop__GoldConversion__Enabled': 'true' if purchases else 'false',
