...
import sys
import time
from prepare_backend import ROOT, digest
...
def main():
    ...
    if not preparation:
        ...
        for service in plan['activeBefore']['services']:
            if service == 'arthas-authserver': continue
    ...
    configs.update(str(x) for x in Path('/opt/arthas-next/candidates/modules-update-20260905T1016Z/server/etc/modules').glob('*') if x.is_file())
    hashes = {}
    ...
    report = {'createdAtUnix': int(time.time()), 'dump': str(dump), 'sha256': digest(dump),
        ...
        'warning': 'Online nontransactional tables are not a consistent restore point. Refresh after the authorized stop before migration. Never restore over later purchases or gameplay.'}
