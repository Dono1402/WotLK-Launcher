...
public sealed record ShopSnapshot(int SchemaVersion, string CatalogRevision, DateTimeOffset ObservedAtUtc,
    ...
    ShopGoldConversions? Conversions = null)
{
    ...
    public void Validate()
    {
        ...
        {
            if (History.Count > MaximumHistoryEntries) throw new InvalidDataException("Oversized shop history.");
            HashSet<string> transactions = new(StringComparer.Ordinal);
            foreach (ShopTransaction transaction in History)
            {
                if (transaction is null || !transactions.Add(transaction.Id)) throw new InvalidDataException("Invalid shop history.");
