            ...
            async Task WorkAsync()
            {
                ...
                try
                {
                    ...
                    if (goldOnly)
                    {
                        ...
                        Check(errors.Messages.Count == 0, "No real-conversion binding errors: " + string.Join("\n", errors.Messages));
                        Console.WriteLine("Gold conversion WPF PASS: real action path, pending draft locks, navigation during send, authoritative receipt, FR/EN and reusable bindings; inactive offscreen fixture only.");
                        result = 0; return;
                    }
                    if(renameOnly)
                    {
