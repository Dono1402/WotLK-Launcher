...
(async () => {
  ...
  state.ownCharacters={status:'loading',characters:[],error:null};await apply(state);check('Owned-character loading uses one indicator without a text line',await page.locator('#armory-picker-status .inline-busy').isVisible()&&await page.locator('#armory-picker-status').innerText()===''&&await page.locator('#armory-picker').getAttribute('aria-busy')==='true');
