#!/usr/bin/env python3
"""Open rename purchases and gold conversion only after successful live checks; restore closed flags on failure."""
import fcntl
import json
import os
import subprocess
...
def main():
    ...
    ops = json.loads((ROOT / 'plan.json').read_text())
    ...
    try:
        ...
        wait_for('API healthy with rename purchases and gold conversion enabled', health, 90)
        ...
        print('PASS: native rename purchases and gold conversion enabled and verified through authenticated HTTPS; zero real orders or conversions created.', flush=True)
