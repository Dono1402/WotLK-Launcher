(function (global) {
  ...
  function renderConversations() {
    ...
    if (search && filter === 'all') {
      const contacts = (snapshot.state.contacts || []).filter(contact => [contact.username, contact.characterName].some(value => String(value || '').toLocaleLowerCase().includes(search)) && !(snapshot.state.threads || []).some(thread => thread.kind === 'direct' && peer(thread)?.accountId === contact.accountId));
      if (contacts.length) rows.push({ kind: 'section', key: 'section:contacts', label: 'contacts' });
      for (const contact of contacts) rows.push({ kind: 'contact', key: 'contact:' + contact.accountId, contact });
    }
    R.reconcileKeyed($('conversation-list'), rows, row => row.key, row => {
      ...
      const text = e('span', 'conversation-row-text'); text.append(e('span', 'conversation-row-title', title), e('span', 'conversation-row-preview', row.contact ? R.presenceText(profile, t) : conversationPreview(thread)));
      const meta = e('span', 'conversation-row-meta'); meta.append(e('span', 'conversation-time', thread ? shortDate(thread.lastMessage?.createdAt) : ''));
