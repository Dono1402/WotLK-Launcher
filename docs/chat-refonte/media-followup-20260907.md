...
**Activation serveur en attente d'accord.** Le client local contient les nouvelles fonctionnalités et le registre étendu, mais l'envoi des extensions ajoutées exige ce nouveau serveur. Le candidat n'a pas encore été exécuté sur Linux ni déployé en production. L'activation prévoit une brève interruption de l'API du launcher, sans nouvelle migration et sans redémarrage du serveur de jeu ou d'Hermes. Les corrections visuelles et la suppression ciblée utilisent les routes déjà disponibles.
