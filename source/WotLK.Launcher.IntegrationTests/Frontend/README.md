...
| `ATLAS_CHAT_TEST_OUTPUT` | `artifacts/atlas-chat-polish-20260907/dom`, relatif à la racine du dépôt. Accepte également un chemin absolu. |
...
La suite écrit les captures PNG et `results.json` dans le répertoire de sortie. Ces fichiers générés, profils et dépendances ne font pas partie des sources de tests.

Les 89 assertions couvrent notamment les statuts et avatars, les images sans cadre, les suppressions, le sélecteur Armory, les brouillons, les pièces jointes, les groupes, les médias, les accusés de lecture et le défilement. Elles vérifient également le signal `composerState` transmis au natif lors de l’entrée et de la sortie d’édition, des changements de fil ou de session et de la révocation du droit d’écrire. Le pont étant simulé, ces vérifications ne remplacent pas un dépôt Explorer dans le launcher réel ou une connexion à deux comptes réels.
