...
internal static class InstallerRuntimeTests
{
    ...
    {
        EmbeddedInstallerPayloadSource payload = new();
        True(payload.Length > 0, "Le payload 1.5.0 doit contenir le launcher canonique.");
        Equal(
        ...
        Equal(
            "Cet assistant va installer Atlas Launcher 1.5.0 sur cet ordinateur.",
    ...
    {
        Equal("1.5.0.0", FileVersionInfo.GetVersionInfo(setupArtifact).FileVersion,
    ...
    private static async Task RunElevatedInstallCyclesAsync(string setupArtifact)
    {
            ...
            bool invokeInstalledUninstaller)
        {
            ...
            Equal("1.5.0.0", FileVersionInfo.GetVersionInfo(result.LauncherPath).FileVersion,
