...
Brouillons persistés par environnement et compte, idempotence conservée après redémarrage, attente hors ligne annulable avant POST, progression/retry/annulation des fichiers. Les quatre statuts En ligne, Absent, Ne pas déranger et Apparaître hors ligne se choisissent dans le menu du profil, en haut à droite des onglets, avec une pastille. Le statut En ligne passe automatiquement à Absent après vingt minutes d'inactivité et revient à En ligne lors de la reprise d'activité. Un statut choisi manuellement reste manuel. Messages ne contient plus de bouton de statut, de Réglages des messages ni d'Aide Markdown. Les préférences et le rendu Markdown restent compatibles avec les données existantes. Le curseur lu privé reste indépendant de l'accusé partagé. **Son Messages désactivé jusqu'à choix définitif de l'utilisateur.**
...
Les images envoyées épousent leur propre contour avec des coins arrondis : aucun cadre de pièce jointe ni bloc de nom/taille de fichier autour de l'image. Les messages supprimés sont retirés du rendu, y compris des extraits, sans phrase de remplacement « Ce message est supprimé » ; leurs identifiants restent pris en compte pour la synchronisation et le curseur de lecture.
...
La commande de partage s'intitule « Partager l'Armory d'un personnage ». Elle permet de sélectionner directement un personnage du compte connecté dans un panneau intégré à Messages, sans saisie d'identifiant ni petite fenêtre système. Les autres types de cartes ne sont plus proposés ; leurs anciens messages restent lisibles. Ne pas inventer des statistiques absentes. Continuité du fil liée au compte ; personnage actif dans l'en-tête et, lorsque connu, personnage d'envoi conservé sur le message en jeu.
