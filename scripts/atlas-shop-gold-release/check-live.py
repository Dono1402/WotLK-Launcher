...
def main():
    ...
    try:
        rows = query("START TRANSACTION; INSERT INTO arthas_auth.account(username,salt,verifier,email,reg_mail,joindate,expansion) "
            ...
            f"VALUES(UNHEX('{session}'),@canary,UNHEX('{token_hash}'),UNHEX('{secrets.token_hex(32)}'),'Atlas release 1.7.2 read check',"
        ...
        conversions = shop.get('conversions', {})
        if conversions.get('available') != (phase != 'closed') or conversions.get('requests') != []: raise RuntimeError('Gold conversion availability differs')
        report['goldConversionAvailable'] = conversions['available']
        ...
        expected_id = 'atlas-launcher-1-7-2' if phase == 'published' else 'atlas-launcher-1-7-1'
        ...
        if query(f'SELECT COUNT(*) FROM arthas_auth.atlas_shop_gold_conversion WHERE account_id={account_id};') != [['0']]: raise RuntimeError('Unexpected canary conversion')
        ...
        if owned:
            ...
            if query(f'SELECT COUNT(*) FROM arthas_auth.atlas_shop_gold_conversion WHERE account_id={owned_id};') != [['0']]: raise RuntimeError('Refuse deleting a canary with a conversion')
